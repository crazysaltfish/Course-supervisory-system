...
public class AdminController {
    ...
    public String adminself(Model model,HttpServletRequest request,HttpServletResponse response) throws Exception {
    	//hub-获取到当前登陆账号的用户
    	String username = "";
		...
		//hub-
		Userlogin findByName = userloginService.findByName(username);
		//0是管理员，1是学生，2是老师
		Object resultPerson = new Object();
		if(findByName.getRole()==0) {//如果是管理员
			//如果是管理员，没有信息
		}else if(findByName.getRole()==1) {//如果是学生
			resultPerson = studentService.findById(Integer.parseInt(username));
		}else if(findByName.getRole()==2) {//如果是老师
			resultPerson = teacherService.findById(Integer.parseInt(username));
		}
		model.addAttribute("userlogin", findByName);
       
        return "admin/showSelf";
